...
```text
capacity = 2048 records
canonical_size = 8192 bytes
```
...
offset. The remaining fixed-capacity entries are zero-padded.
...
3. Sort the entire sequence lexicographically by the normalized,
   case-insensitive complete relative path:
...
4. Use normalization only for comparison. Preserve the stored path spelling,
   casing, and existing separator policy; do not use basename-only or natural
   numeric sorting.
